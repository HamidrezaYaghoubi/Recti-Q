...
#SBATCH --job-name=ft_bdd_yolov8n
...
CONFIG_PATH="${CONFIG_PATH:-configs/finetune_bdd100k_yolo.yaml}"
DEVICE_STRING="${DEVICE_STRING:-0,1,2,3}"
TIMESTAMP=$(date -u +"%Y%m%dT%H%M%SZ")
RUN_LOG="$ROOT_DIR/logs/finetune_bdd_yolov8n_${TIMESTAMP}.log"

echo "Root dir: $ROOT_DIR"
echo "Using config: $CONFIG_PATH"
echo "Using device: $DEVICE_STRING"
echo "Logging run output to: $RUN_LOG"
echo "Visible GPUs:"
nvidia-smi --list-gpus || true

"$PYTHON_BIN" -m src.finetune \
  --config "$CONFIG_PATH" \
  --models yolov8n \
